...
export function WrappedApp({ data }: WrappedAppProps) {
  ...
  const statsRevealProgress = easeInOutCubic(
    Math.max(0, (scroll.stats - 0.2) / 0.8)
  );
  const statsLiftProgress = easeInOutCubic(scroll.statsExit);
  const statsVisibility = statsRevealProgress * (1 - statsLiftProgress);

  return (
    <main className="relative min-h-[670vh] overflow-x-clip bg-magic text-cream">
      ...
      <div
        className="pointer-events-none fixed inset-0 z-20 bg-vignette"
        aria-hidden="true"
      />
      <div
        className="pointer-events-none fixed inset-0 z-20 bg-grain opacity-25"
        aria-hidden="true"
      />
      ...
      <p
        ...
        style={{
          opacity: scroll.morph >= 0.95 && scroll.thankYou < 0.9 ? 1 : 0,
        }}
      >
        {data.hero.hint}
      </p>
      ...
      <div className="relative z-[45] mx-auto max-w-4xl space-y-5 px-4 pb-8 pt-[530vh] md:space-y-6 md:px-6">
        ...
        />
        <LanguagesSection
          title={data.particle.title}
          items={data.particle.languages}
        />
        <ProjectsSection
